...
namespace callisto {
	struct Project {
		std::string name{};
		fs::path callisto_executable{};
		fs::path project_root{};
		std::chrono::system_clock::time_point last_accessed{};

		bool operator==(const Project& other) {
			return name == other.name && callisto_executable == other.callisto_executable &&
				project_root == other.project_root;
		}

		Project& operator=(const Project& other) {
			name = other.name;
			callisto_executable = other.callisto_executable;
			project_root = other.project_root;
		...
		Project(const fs::path& callisto_executable, const fs::path& project_root)
			: name(project_root.stem().string()), callisto_executable(callisto_executable), project_root(project_root),
		...
		json toJson() const {
			...
			j["name"] = name;
			j["callisto_executable"] = callisto_executable;
		...
		Project(const json& j) : name(j["name"].get<std::string>()), callisto_executable(j["callisto_executable"].get<std::string>()),
	...
	public:
		...
		void update(const fs::path& project_root, const fs::path& callisto_executable) {
			reloadList();

			Project project{ callisto_executable, project_root };
		...
		void removeInvalidProjects() {
			std::erase_if(projects, [](const Project& project) {
				return !fs::exists(project.project_root) || !fs::exists(project.callisto_executable);
			});
