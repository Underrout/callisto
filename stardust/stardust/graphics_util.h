	...
	protected:
		...
		static void deleteSymlink(const fs::path& symlink_path) {
			...
			try {
				...
				throw StardustException(fmt::format("Failed to delete symlink {}", symlink_path.string()));
			}
#else
			try {
				...
				throw StardustException(fmt::format("Failed to delete junction {}", symlink_path.string()));
			}
#endif
		}
	...
	public:
		...
		static void createSymlink(const fs::path& link_name, const fs::path& target_name) {
			...
			try {
				...
				throw StardustException(fmt::format("Failed to create symlink {} -> {}", link_name.string(), target_name.string()));
			}
#else
			try {
				if (fs::exists(link_name)) {
				...
				throw StardustException(fmt::format("Failed to create junction {} -> {}", link_name.string(), target_name.string()));
			}
#endif
		}
